...
void UMaterial::PostLoad()
{
	...
	DoMaterialAttributeReorder(&SpecularOffset, UE4Ver, RenderObjVer);
	DoMaterialAttributeReorder(&SpecularRange, UE4Ver, RenderObjVer);
	...
	bool bUsesShadingModelFromMaterialExpression)
{
	...
	case MP_CustomData0:
		...
		break;
	case MP_CustomData1:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ClearCoat, MSM_Eye, MSM_Toon, MSM_ToonSkin, MSM_ToonHair, MSM_ToonAniso});
		break;
	...
	case MP_SpecularRange:
		...
		break;
	case MP_ShadowColor:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ToonSkin, MSM_ToonAniso});
		break;
