...
enum EMaterialShadingModel
{
	...
	MSM_ToonAniso			UMETA(DisplayName = "Toon Aniso"),
...
static_assert(MSM_NUM <= 17, "Do not exceed 16 shading models without expanding FMaterialShadingModelField to support uint32 instead of uint16!");
...
public:
	...
	void AddShadingModel(EMaterialShadingModel InShadingModel)				{ check(InShadingModel < MSM_NUM); ShadingModelField |= ((uint32)1 << (uint32)InShadingModel); }
	void RemoveShadingModel(EMaterialShadingModel InShadingModel)			{ ShadingModelField &= ~((uint32)1 << (uint32)InShadingModel); }
	...
	bool HasShadingModel(EMaterialShadingModel InShadingModel) const		{ return (ShadingModelField & ((uint32)1 << (uint32)InShadingModel)) != 0; }
	bool HasOnlyShadingModel(EMaterialShadingModel InShadingModel) const	{ return ShadingModelField == ((uint32)1 << (uint32)InShadingModel); }
	bool IsUnlit() const													{ return HasShadingModel(MSM_Unlit); }
	bool IsLit() const														{ return !IsUnlit(); }
	bool IsValid() const													{ return (ShadingModelField > 0) && (ShadingModelField < ((uint32)1 << MSM_NUM)); }
