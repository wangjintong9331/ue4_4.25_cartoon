...
struct FMaterialAttributesInput : FExpressionInput
{
	...
	{ 
		...
		static_assert((uint64)(MP_MAX)-1 <= (8 * sizeof(PropertyConnectedBitmask)), "PropertyConnectedBitmask cannot contain entire EMaterialProperty enumeration.");
	}
	...
	{
		PropertyConnectedBitmask = bIsConnected ? PropertyConnectedBitmask | ((uint64)1 << (uint64)Property) : PropertyConnectedBitmask & ~((uint64)1 << (uint64)Property);
	}
